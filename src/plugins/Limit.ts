              ...
              if (this.options.limits[nodeType]) {
                if (this.options.limits[nodeType].charCount) {
                  ...
                  if (charCount > this.options.limits[nodeType].charCount) {
                    decorations.push(Decoration.inline(pos + this.options.limits[nodeType].charCount, pos + node.nodeSize, {
                      'data-limit': 'char',
                      class: 'unvalid'
                    }))
                  }
                } else if (this.options.limits[nodeType].wordCount) {
                  const counter = node.textContent.split('').reduce((acc, cur) => {
                    if (cur == ' ') {
                      acc.words++
                    }
                  ...
                  if (counter.words >= this.options.limits[nodeType].wordCount) {
                    decorations.push(Decoration.inline(pos + counter.chars, pos + node.nodeSize, {
                      'data-limit': 'word',
                      class: 'unvalid'
                    }))
                  }
