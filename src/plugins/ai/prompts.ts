...
// et le moteur local (Web Worker). Module PUR (aucun import) pour être consommable
// des deux côtés sans dupliquer les templates.
export type Task = 'autocomplete' | 'shorten' | 'alternative'

export function buildPrompt(task: Task, text: string, context?: string): string {
  switch (task) {
    case 'autocomplete':
      return `You are an artificial intelligence designed to assist a journalist in writing an article by completing their sentences.
Instructions:
...
Text: ${text}`
    case 'shorten':
      return `You are an AI editing assistant for a journalist. Your task is to propose a shortened version of the selected text while preserving its meaning and accuracy.
Rules:
...
Text: ${text}`
    case 'alternative':
      return `You are an AI editing assistant for a journalist. Your task is to suggest a synonym when a single word is selected, or a rephrased alternative when a group of words is selected, while preserving the original meaning and intention.
Rules:
