...
export function getVectorStore (embeddings: EmbeddingsInterface) {
  ...
  return {
    addDocuments: async (documents: Document[]): Promise<DocumentsWithVectors[]> => {
    ...
    addVectors: (documentsWithVectors: DocumentsWithVectors[]) => {
      return vectorStore.addVectors(documentsWithVectors.map(d => d.vectors), documentsWithVectors)
    },
